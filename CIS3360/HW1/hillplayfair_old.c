...
int main(int argc, char *argv[]) {
    ...
    */ //starting point
    ...
    p_ciphertext[s_len + k + 1] = '\0'; //adding the null terminator
    ...
    for (m = 0; m < pad_len; m = m + dim) {
        ...
        for (n = 0; n < dim; n++) {
            ...
            c_ciphertext[m + n] = (char)((pmat[n][0] % 26) + 65); //convert the results back into char, mod 26 to make sure the numbers are in range
        }
    ...
    while (a < c_len) {
        ...
        if (next_c == c) { //checking for repeated letters
            ...
            pc_ciphertext = grow_string(pc_ciphertext, b + 1);
            if (c == 'X') { //checking last character to pad appropriately
                pc_ciphertext[b] = 'Z'; 
            } else {
                pc_ciphertext[b] = 'X';
            }
    ...
    int rindex2 = 0;
    int cindex2 = 0;
    char *dc_ciphertext = malloc(2); //final string
    ...
    for (g = 0; g < pc_len; g += 2) {
        ...
        if (rindex1 == rindex2) { //same row
            cindex1++;
            cindex2++;
            if (cindex1 == 5) { //wraping around
                cindex1 = 0;
            }
            if (cindex2 == 5) {
                cindex2 = 0;
            }
            dc_ciphertext = grow_string(dc_ciphertext, g + 2); //space for both chars
            dc_ciphertext[g] = play_array[rindex1][cindex1];
            dc_ciphertext[g+1] = play_array[rindex2][cindex2];
        } else if (cindex1 == cindex2) { //similar to the row rule above
            rindex1++;
            rindex2++;
            if (rindex1 == 5) { //wraping around
                rindex1 = 0;
            }
            if (rindex2 == 5) {
                rindex2 = 0;
            ...
            dc_ciphertext = grow_string(dc_ciphertext, g + 2);
            dc_ciphertext[g] = play_array[rindex1][cindex2];
            dc_ciphertext[g+1] = play_array[rindex2][cindex1];
        }
    }

    dc_ciphertext[pc_len] = '\0';
